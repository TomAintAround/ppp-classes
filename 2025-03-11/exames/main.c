...
void printClassificacoes(int tamanho, Exame classificacoes[tamanho]) {
	for (int i = 0; i < tamanho; i++) {
		printf("Nome: %s, época: %s, classificação: %d ", classificacoes[i].nome, classificacoes[i].epoca, classificacoes[i].nota);
...
void frequenciasAbsolutas(int tamanho, Exame classificacoes[tamanho], int notas[], char epoca[]) {
	for (int i = 0; i < 11; i++) {
		notas[i] = 0;
	}
	for (int i = 0; i < tamanho; i++) {
